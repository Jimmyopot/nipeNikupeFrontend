      ...
      <Box sx={{ maxWidth: 1200, mx: 'auto', px: 2 }}>
        <Box sx={{ textAlign: 'center', mb: 8 }}>
          ...
          <Typography variant="h6" sx={{ color: 'text.secondary', maxWidth: 700, mx: 'auto' }}>
            Real stories from real people who have transformed their skills into valuable connections.
          </Typography>
        </Box>

        <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', md: 'repeat(3, 1fr)' }, gap: 4, maxWidth: 1000, mx: 'auto' }}>
          {testimonials.map((testimonial, index) => (
            <Card key={index} sx={{ position: 'relative', borderRadius: 3 }} elevation={0}>
              <CardContent sx={{ p: 4, position: 'relative' }}>
                {/* Quote Icon */}
                <Box sx={{ position: 'absolute', top: -18, left: 24, zIndex: 2 }}>
                  <Box sx={{ bgcolor: 'primary.main', width: 32, height: 32, borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                    <FormatQuoteIcon sx={{ color: 'primary.contrastText', fontSize: 18 }} />
                  </Box>
                </Box>
                ...
                {/* Content */}
                <Typography variant="body1" sx={{ color: 'text.secondary', mb: 3, fontStyle: 'italic', lineHeight: 1.7 }}>
                  "{testimonial.content}"
                </Typography>
