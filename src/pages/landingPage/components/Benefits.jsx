...
export function BenefitsSection() {
  const benefits = [
    {
      icon: <ShieldIcon sx={{ fontSize: 28, color: 'primary.main' }} />,
    ...
    {
      icon: <StarIcon sx={{ fontSize: 28, color: 'primary.main' }} />,
      title: "Quality Assurance",
      description:
        "Our rating system ensures high-quality exchanges and helps you find the best matches.",
    },
    {
      icon: <GroupIcon sx={{ fontSize: 28, color: 'primary.main' }} />,
      ...
      <Box sx={{ maxWidth: 1400, mx: 'auto', px: 2 }}>
        <Box sx={{ textAlign: 'center', mb: 8 }}>
          ...
          <Typography variant="h6" sx={{ color: 'text.secondary', maxWidth: 700, mx: 'auto' }}>
            Experience the benefits of a trusted community built on mutual growth and skill sharing.
          </Typography>
        </Box>

        <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', md: '1fr 1fr', lg: 'repeat(3, 1fr)' }, gap: 3, maxWidth: 1000, mx: 'auto' }}>
            ...
            >
              <CardContent sx={{ p: 4, textAlign: 'center' }}>
                <Box sx={{ mb: 3, display: 'flex', justifyContent: 'center' }}>
                  <Box sx={{ width: 48, height: 48, bgcolor: 'primary.light', borderRadius: 2, display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'background 0.2s' }}>
                    {benefit.icon}
                  </Box>
                </Box>
                <Typography variant="h6" sx={{ fontWeight: 600, mb: 1 }}>{benefit.title}</Typography>
                <Typography variant="body2" sx={{ color: 'text.secondary', lineHeight: 1.7 }}>{benefit.description}</Typography>
              </CardContent>
            </Card>
          ))}
        </Box>
