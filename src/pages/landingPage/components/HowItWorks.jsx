      ...
      <Box sx={{ maxWidth: 1200, mx: 'auto', px: 2 }}>
        <Box sx={{ textAlign: 'center', mb: 8 }}>
          ...
          <Typography variant="h6" sx={{ color: 'text.secondary', maxWidth: 600, mx: 'auto' }}>
            Getting started with skill exchange is simple. Follow these three easy steps to begin building your network.
          </Typography>
        </Box>

        <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', md: 'repeat(3, 1fr)' }, gap: 4, maxWidth: 1000, mx: 'auto' }}>
          {steps.map((step, index) => (
            <Card
              key={index}
              sx={{ position: 'relative', border: 2, borderColor: 'divider', 
                transition: 'border-color 0.2s', '&:hover': { borderColor: 'primary.main' }, borderRadius: 3 }}
              elevation={0}
            >

              <CardContent sx={{ p: 5, textAlign: 'center', minHeight: 300, position: 'relative' }}>
                ...
                {/* Icon */}
                <Box sx={{ mb: 3, mt: 3 }}>
                  <Box sx={{ width: 64, height: 64, bgcolor: 'secondary.light', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', mx: 'auto' }}>
                    {step.icon}
                  </Box>
                </Box>
